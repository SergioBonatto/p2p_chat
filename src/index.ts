...
type Message = {
  type: 'hello' | 'enc'
  peerId?: string
  pubKey?: string // base64 DER spki (only in hello)
  payload?: string
  ts?: number
}

// Internal payload structure (inside encrypted data)
type EncryptedPayload = {
  peerId: string
  text: string
  ts: number
  nonce: string // unique nonce to prevent replay
  seq?: number // sequence number for ordering (future use)
}
...
// ----- Helpers -----
function msgToBuffer(obj: Message): Buffer {
  return Buffer.from(JSON.stringify(obj) + '\n', 'utf8')
}

function parseLines(buffered: Buffer, carry: string): { lines: string[]; carry: string } {
...
function encryptWithSignature(plaintext: string, key: Buffer, signKey: crypto.KeyObject): string {
...
function decryptAndExtractSignature(base64payload: string, key: Buffer): { plaintext: string; signature: Buffer } {
  const buf = Buffer.from(base64payload, 'base64')
  if (buf.length < 12 + 16 + 64) throw new Error('payload too short')
...
function saveFileSecure(p: string, content: string | Buffer) {
  // write file and set permission 600
  fs.writeFileSync(p, content, { mode: 0o600 })
}

function loadPrivateKeyOrCreate(): { privateKey: crypto.KeyObject; publicKey: crypto.KeyObject; created: boolean } {
  ensureStorageDir()
...
function loadOrCreatePeerId(): { peerId: string; created: boolean } {
  ensureStorageDir()
  if (fs.existsSync(PEERID_PATH)) {
    ...
    try { fs.chmodSync(PEERID_PATH, 0o600) } catch { /* ignore */ }
...
async function main() {
  ...
  const peerPubKeys = new Map<string, crypto.KeyObject>()

  // store nonces per peer to prevent replay attacks
  // Map<peerId, Set<nonce>>
  const peerNonces = new Map<string, Set<string>>()
  ...
  swarm.on('connection', (socket: Socket, details: any) => {
        ...
        try {
          ...
          if (m.type === 'hello') {
            ...
            try {
              ...
              try {
                parsed = JSON.parse(plaintext) as EncryptedPayload
              } catch {
